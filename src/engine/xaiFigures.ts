 ...
 *
 * Cizim `components/XaiFigure.tsx` icinde, burasi yalnizca sayi uretir; boylece
 * test edilebilir kalir. Gorsel senaryonun kendi enjeksiyon tanimindan turer,
 * boylece hangi kanal secildiyse gorsel de o kanali gosterir.
 *
...
/**
 * Bir kanitin hangi senaryodan geldigini `asset` alaninin senaryo zaman
 * cizelgesindeki show_xai adimiyla eslestirerek bulur.
 *
 * Bu yol bilerek secildi: `XaiEvidence` tipine senaryo kimligi eklemek
 * `engine/types.ts`'i degistirmek demekti. O dosya projenin ortak tipler
 * dosyasi; baska dallarla cakisma riskini dusuk tutmak icin dokunulmadi.
 */
export function scenarioByAsset(asset: string): Scenario | null {
  for (const sc of SCENARIOS) {
    for (const st of sc.timeline) {
      if (st.type === 'show_xai' && st.asset === asset) return sc;
    }
  }
  return null;
}
...
export function injectedChannels(sc: Scenario): string[] {
  const out: string[] = [];
  for (const st of sc.timeline) {
    if (st.type === 'inject_point' || st.type === 'inject_drift') {
      if (!out.includes(st.pid)) out.push(st.pid);
    } else if (st.type === 'inject_collective') {
      for (const t of st.targets) if (!out.includes(t.pid)) out.push(t.pid);
    }
  }
  return out;
}
