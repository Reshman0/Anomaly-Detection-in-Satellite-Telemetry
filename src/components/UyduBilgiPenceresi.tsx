 ...
 *
 * Acik/kapali durumu bilesenin kendi icinde tutulur ve pencere document.body'ye
 * portal ile basilir. Boylece store.ts'e ve App.tsx'e yeni alan eklenmez.
 */
    ...
    <>
      <div className="flex items-center px-2">
        <button
          ...
          title="Seçili uydunun bilgileri"
          className="flex items-center gap-1 px-2 py-[4px] border border-ops-line2 text-ops-dim hover:text-ops-text hover:border-ops-dim transition-colors"
        >
          <span className="text-[13px] leading-[15px]">ⓘ</span>
          <span className="text-[10px] uppercase tracking-[0.14em] leading-[13px]">Bilgi</span>
        </button>
      </div>
      {acik && createPortal(<Pencere kapat={kapat} />, document.body)}
    </>
  );
}
